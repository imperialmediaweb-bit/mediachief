            ...
            <div class="lg:col-span-2">
                                ...
                                <a href="{{ route('article.show', $topArticle) }}" class="block">
                                    <div class="relative aspect-video overflow-hidden">
                                        @if($topArticle->image_url)
                                            <img src="{{ $topArticle->image_url }}" alt="{{ $topArticle->title }}" class="article-card-img h-full w-full object-cover">
                                        @else
                                            <div class="h-full w-full bg-gray-200"></div>
                                        @endif
                                        <div class="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent"></div>
                                        <div class="absolute bottom-0 left-0 p-4">
                                            <span class="cat-badge mb-2">{{ $category->name }}</span>
                                            <h2 class="text-base font-bold leading-tight text-white">{{ Str::limit($topArticle->title, 70) }}</h2>
                                        </div>
                                    </div>
                                </a>
                            </div>
                ...
                <div class="space-y-6">
                    ...
                    @forelse($articles->skip($skipCount) as $article)
                        <article class="flex gap-5 border-b border-gray-100 pb-6">
                            @if($article->image_url)
                                <a href="{{ route('article.show', $article) }}" class="shrink-0">
                                    <img src="{{ $article->image_url }}" alt="{{ $article->title }}" class="h-28 w-44 object-cover md:h-36 md:w-56">
                                </a>
                            @endif
                            <div class="min-w-0 flex-1">
                                ...
                                @if($article->excerpt)
                                    <p class="mb-3 hidden text-sm text-gray-600 md:block">{{ Str::limit($article->excerpt, 160) }}</p>
                                @endif
                                <div class="flex items-center gap-3 text-xs text-gray-500">
                                    @if($article->author)<span>{{ $article->author }}</span><span>&middot;</span>@endif
