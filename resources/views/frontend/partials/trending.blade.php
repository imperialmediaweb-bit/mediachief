        ...
        <div class="flex h-[38px] items-center gap-3 overflow-hidden">
            ...
            <div class="relative h-[38px] flex-1 overflow-hidden">
                @foreach($trendingArticles as $i => $trending)
                    <a href="{{ route('article.show', $trending) }}" class="td-ticker-item absolute inset-0 flex items-center text-[13px] text-gray-700 hover:text-brand-red {{ $i === 0 ? 'opacity-100' : 'opacity-0' }}">
