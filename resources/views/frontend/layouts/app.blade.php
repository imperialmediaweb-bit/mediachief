...
@else
{{-- ═══ Newspaper Theme Mode: Using Newspaper CSS directly ═══ --}}
<!DOCTYPE html>
...
<head>
    ...
    {{-- Google Fonts (Newspaper theme fonts) --}}
    ...
    {{-- Newspaper Theme CSS --}}
    <link rel="stylesheet" href="{{ asset('css/wp-theme/newspaper-style.css') }}">
    <link rel="stylesheet" href="{{ asset('css/wp-theme/newspaper-icons.css') }}">

    @php
        $theme = $currentSite->settings['theme'] ?? [];
        $analytics = $currentSite->analytics ?? [];
        $themeColor = $theme['primary_color'] ?? '#4db2ec';
    @endphp

    <style>
        :root {
            --td_theme_color: {{ $themeColor }};
            --td_grid_border_color: #ededed;
            --td_black: #222;
            --td_text_color: #111;
            --td_header_color: #222;
            --td_text_header_color: #fff;
        }
    </style>
...
<body class="tdc-theme-Newspaper td-header-style-1">
